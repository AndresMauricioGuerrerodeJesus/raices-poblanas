...
---------------------------------------------------------------
-------------------------------------------------------v1.1 
-- Ejecuta este script desde cero en pgAdmin
DROP DATABASE IF EXISTS db_raices_poblanas;
CREATE DATABASE db_raices_poblanas;



-- Conéctate a la nueva BD y ejecuta lo siguiente:
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

CREATE TABLE roles (
    role_id BIGSERIAL PRIMARY KEY, -- BIGSERIAL = bigint (8 bytes) para coincidir con Long
...
CREATE TABLE users (
    ...
    role_id BIGINT REFERENCES roles(role_id) NOT NULL, -- BIGINT para llaves foráneas
    username VARCHAR(50) NOT NULL UNIQUE,
    email VARCHAR(100) NOT NULL UNIQUE,
...
CREATE TABLE products (
    ...
    unique_piece_id UUID DEFAULT uuid_generate_v4() UNIQUE, 
    artisan_id BIGINT REFERENCES artisans(artisan_id) NOT NULL,
...
CREATE TABLE orders (
    ...
    customer_id BIGINT REFERENCES users(user_id) NOT NULL,
    total_amount NUMERIC(12,2) NOT NULL,
...
-- Insertar un Usuario Artesano (Password: password123 en texto plano para prueba rápida)
INSERT INTO users (username, email, password_hash, role_id, wallet_balance) 
VALUES ('maury_artesano', 'maury@sierra.com', '$2a$10$8.UnS3pWDExX09X/I9mN/uX9xR8zG3I/Q/4G5g7vG3YmZ9G1l2a3b', 2, 0.00);

-- Crear Perfil de Artesano
INSERT INTO artisans (user_id, bio, municipality, is_verified) 
VALUES (1, 'Maestro artesano de textiles en telar de cintura.', 'Teziutlán', TRUE);

-- Insertar Categorías
INSERT INTO categories (name) VALUES ('Textiles'), ('Cerámica'), ('Madera');

-- Insertar un Producto de prueba para el Catálogo
INSERT INTO products (artisan_id, category_id, name, collection_name, description, materials, price, status) 
VALUES (1, 1, 'Blusa Bordada de Teziutlán', 'Colección Primavera', 'Bordado a mano con hilos de seda.', 'Algodón', 850.00, 'Available');
